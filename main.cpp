...
int userMove();
void makeMove(vector<vector<int> >&, int, int);
...
array<int, 2> miniMax(vector<vector<int> >&, int, int, int, int);
int heurFunction(int, int, int);

// I'll be real and say this is just to avoid magic numbers
int NUM_COL = 7; // how wide is the board
int NUM_ROW = 6; // how tall
int PLAYER = 1; // player number
int COMPUTER = 2; // AI number
int MAX_DEPTH = 5; // the default "difficulty" of the computer controlled AI

bool gameOver = false; // flag for if game is over
int turns = 0; // count for # turns
int currentPlayer = PLAYER; // current player
 ...
 */
void makeMove(vector<vector<int> >& b, int c, int p) {
...
int userMove() {
	...
	while (true) { // repeat until proper input given
		...
		else if (!(move-1 < NUM_COL && move-1 >= 0)) { // if out of bounds
			errorMessage(2); // let user know
		}
		else if (board[NUM_ROW - 1][move-1] != 0) { // if full column
 ...
 */
array<int, 2> miniMax(vector<vector<int> > &b, int d, int alf, int bet, int p) {
	/**
	...
	if (p == COMPUTER) { // if AI player
		...
		for (int c = 0; c < NUM_COL; c++) { // for each possible move
 ...
 */
int tabScore(vector<vector<int> > b, int p) {
	int score = 0;
	vector<int> rs(NUM_COL);
	vector<int> cs(NUM_ROW);
	vector<int> set(4);
	/**
 ...
 */
bool winningMove(vector<vector<int> > &b, int p) {
	int winSequence = 0; // to count adjacent pieces
	// for horizontal checks
	for (int c = 0; c < NUM_COL - 3; c++) { // for each column
		for (int r = 0; r < NUM_ROW; r++) { // each row
			for (int i = 0; i < 4; i++) { // recall you need 4 to win
				if (b[r][c + i] == p) { // if not all pieces match
